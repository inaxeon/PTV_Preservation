...
// The standard PM5644 CPU code doesn't allow the full PM8546 character set to be used.
code promblock_t _g_char_blocks[] = {
    ...
    { 0, 0x00 }, // NOT ALLOWED: '}'
    { 0, 0x00 }, // NOT ALLOWED: '~'
    { 11, 0xF0 }  // NOT ALLOWED: 0x7F (PTV Logo)
};
...
void logogen_update_text(uint8_t field)
{
    if (field == TEXTA || field == TEXTB)
    {
        ...
        if (len)
        {
            uint8_t odd = (len & 1) ? 0 : 1;
            offset = (CENTER + odd) - (len / 2);
