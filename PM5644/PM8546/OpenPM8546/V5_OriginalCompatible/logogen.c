...
#define LINES_TO_TEXTA  35
#define LINES_TO_CLOCK  63
...
#define TS_OFF_A        0
#define TS_LINE1_START  1
#define TS_LINE1        2
#define TS_OFF_B        3
#define TS_CLOCK_START  4
#define TS_CLOCK        5
#define TS_OFF_C        6
#define TS_LINE2_START  7
#define TS_LINE2        8
#define TS_OFF_D        9
#define TS_INIT         10
        ...
        case TS_LINE1_START:
            if (_text_flags & LOGOGEN_TEXTA_ON)
            {
                if (_text_flags & LOGOGEN_LOGO1_ON)
                    _vcontrol_reg = VC_START | VC_SEL0 | VC_BANK1; 
                else
                    _vcontrol_reg = VC_START | VC_SEL0;
            }
            _text_state = TS_LINE1;
            TRIGGER0_IN_LINES(0);
            break;
        case TS_LINE1:
            _vcontrol_reg &= ~VC_START;
            _text_state = TS_OFF_B;
            TRIGGER0_IN_LINES(LINE_HEIGHT);
            break;
        case TS_OFF_B:
            _vcontrol_reg = 0;
            _text_state = TS_CLOCK_START;
            TRIGGER0_IN_LINES(LINES_TO_CLOCK);
            break;
        case TS_CLOCK_START:
            if (_text_flags & LOGOGEN_CLOCK_ON)
            {
                _vcontrol_reg = VC_START | VC_SEL1;
                VCONTROL = _vcontrol_reg;
            }
        ...
        case TS_LINE2_START:
            if (_text_flags & LOGOGEN_TEXTB_ON)
            {
                if (_text_flags & LOGOGEN_LOGO2_ON)
                    _vcontrol_reg = VC_START | VC_SEL1 | VC_SEL0 | VC_BANK1;
                else
                    _vcontrol_reg = VC_START | VC_SEL1 | VC_SEL0;
            }
...
static void put_clock_char(uint16_t *dpram_ptr, uint8_t idx)
{
    idx += 0x40; // Offset to '0' character
    idx <<= 1;
