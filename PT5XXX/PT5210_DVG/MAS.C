...
bit WarningLED;            // 1: Illumination of the WARNING LED
bit UnlockedLED;            // 1: Illumination of the UNLOCKED LED
bit GenlockLED;            // 1: Illumination of the GENLOCK LED
...
// (Brug kun 9-F, 0 ell. 8 er intrpt off!)
   ...
   EA = 1 ;					// Enable all interrupts
	...
	WarningLED  = ON;          // Switch ON all LEDs during power-up
	UnlockedLED = ON;
	GenlockLED  = ON;

	UpdateLEDs();

	WarningLED  = OFF;        // Switch OFF all LEDs after LED testing
	UnlockedLED = OFF;
	GenlockLED  = OFF;
	...
	switch ( RdFrontIIC( keyb_addr) & KeybMask) {
		case FACTORYButton:
